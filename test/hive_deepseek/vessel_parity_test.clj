(ns hive-deepseek.vessel-parity-test
  "The standard sample batch (the one hive-vessel.parity-test locks) planned
   for THIS vessel's target and golden-locked here, so a hive-vessel bump that
   changes a :json lowering is a visible diff in this repo. Planning is pure:
   no bridge and no harness page are needed."
...
(defn- natives []
  (let [target (dissoc (vessel/target nil) :vessel/execute!)
        r (v/plan (v/standard-registry) target sample-batch)]
    (is (:ok r) (pr-str (:error r)))
    (mapv :native/payload (get-in r [:ok :plan/ops]))))

(deftest every-sample-op-plans-for-this-vessel
  (is (= :json (:vessel/dialect (vessel/target nil))))
